const urlShowInfo = 'http://api.tvmaze.com/shows/';
const cardHolderShowInfo = $('.cardHolder');
...
const showMovieInfo = (id) =>
    $.ajax({
        ...
        let card = `<h1 class="mb-5 mt-5 text-center">${response.name}</h1>
          ...
          <h2>Show Details</h2>
          ${response.summary}`;
        cardHolderShowInfo.append(card);
    }).fail(() => {
        alert("Network error");
    });
